...
const CopraHome = () => {
  ...
  const fetchTotalTrasaction = async () => {
    try {
      const response = await axios.get(
        "/dashboard/coprahome/total-transaction"
      );
  ...
  const fetchDailyMarketPrice = async () => {
    try {
      const response = await axios.get(
        "/dashboard/coprahome/daily-market-price"
      );
          ...
          {dailyMarketPrice ? (
            <View className="mt-4">
              <PriceSection
                ...
                average={averagePrice ? averagePrice.toString() : "0"} // Add null check with default value
              />
            </View>
          ) : (
            "loading...."
